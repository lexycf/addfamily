...
;
    ...
    var commonsPart = {
        ...
        isDom: function (data) {
            ...
            if (data && data instanceof $) return data[0].outerHTML;
    ...
    DialogViewPro.prototype = {
        ...
        creat: function () {
            ...
            _domCreat.shade = _config.shade ? '<div class="dialogView-m-shade" data-times="' + dialogView.index + '" style="z-index:' + _config.zIndex + '"></div>' : "";
            ...
            if (!this.isConfig) {
                _domCreat.content = '<div class="mobile-dialogView" data-animate="' + _config.animate + '" data-times="' + dialogView.index + '">' + _domCreat.shade + '<div class="dialogView-m-main dialogView-m-aniamte ' + _domCreat.animate + '" style="z-index:' + (_config.zIndex + 1) + '"><div class="dialogView-m-section">' +
                    '<div class="dialogView-m-wrap"><div class="dialogView-m-content">' + (_config.defaultMsg ? _config.defaultMsg : "") + '</div></div></div></div></div>';
            } else {
                _domCreat.content = '<div class="mobile-dialogView ' + (_config.className ? _config.className : "") + '" data-animate="' + _config.animate + '" data-times="' + dialogView.index + '">' + _domCreat.shade + '<div class="dialogView-m-main dialogView-m-aniamte ' + _domCreat.animate + '" style="z-index:' + (_config.zIndex + 1) + '"><div class="dialogView-m-section">' +
            ...
            _domCache = {
                "view": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]'),
                "shade": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]' + ' .dialogView-m-shade'),
                "main": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]' + ' .dialogView-m-main'),
                "section": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]' + ' .dialogView-m-section'),
                "wrap": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]' + ' .dialogView-m-wrap'),
                "content": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]' + ' .dialogView-m-content'),
                "buttons": $('.mobile-dialogView' + '[data-times="' + dialogView.index + '"]' + ' .dialogView-m-buttons')
            };

            this.isConfig ?
                (_config.showTime && $(_domCache.view).data("showTime", _config.showTime)) :
                (_config.defaultTime && $(_domCache.view).data("showTime", _config.defaultTime));
            commonsPart.isArray(_config.buttons) && this.buttonsUI(_config, _domCache);
            this.bindUI(_config, _domCache);

            //console.log($(".mobile-dialogView").data('times'));
        },
        bindUI: function (_config, _domCache) {
            var _this_ = this;
            //设置了showTime值就自动关闭
            $(_domCache.view).data("showTime") && this.close(_domCache.view);
            //点击遮罩层自动关闭
            (function () {
        ...
        buttonsUI: function (_config, _domCache) {
            ...
            (function () {
                ...
                if (_config.type == dialogView.type[1] || _config.type == dialogView.type[3]) {
                    _config.buttons.splice(2, _config.buttons.length - 2);
        ...
        close: function (view, closeType) {
            ...
            var _showTime = closeType ? 0 : (view ? ($(view).data("showTime") || 0) : 0);
            setTimeout(function () {
